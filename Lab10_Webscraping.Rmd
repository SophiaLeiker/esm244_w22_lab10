...
count_end <- sprintf(count_stem, api_key) #api_key is what goes into the %s spots 
...
--> We are replacing the page number for this, so turn page_number into `%s`
```{r}
...
spp_df <- spp_page1$result %>%
  ...
  filter(!is.na(category)) #any category that is na will be dropped out
...
threatened_pct_cells <- spp_risk_cells %>% 
  filter(prob >= 0.5) %>% 
  mutate(threatened = (tolower(cat) %in% c('vu', "en", "cr", "exx"))) %>% 
  filter(cat != 'dd') %>% 
  group_by(lon, lat) %>% 
...
#TO GGPLOT A RASTER USE GEOM_RASTER BUT IT TAKES IN A DATAFRAME NOT A RASTER
